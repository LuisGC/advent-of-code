...
class Burrow:
    ...
    def get_piece_from_room(self, room):
        for char in room:
            if char != '.':
                return char

    def add_to_room(self, char, room):
        room = list(room)
        distance = room.count('.')
        room[distance - 1] = char
        return ''.join(room), distance

    def can_reach(self, cavern, position, destination):
    ...
    def room_ok(self, cavern, piece, destination):
        in_room = cavern[destination]
        return len(in_room) == in_room.count('.') + in_room.count(piece)

    def possible_moves(self, cavern, position):
        ...
        if position == self.dest[moving_letter] and self.room_ok(cavern, moving_letter, position):
            return []
        ...
        for dest in range(len(cavern)):
            if dest == position:
                continue
            if dest in self.destination_set and self.dest[moving_letter] != dest:
                continue
            if self.dest[moving_letter] == dest:
    ...
    def move(self, cavern, position, destination):
        new_cavern = cavern[:]
        distance = 0
