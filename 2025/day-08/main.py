...
def connect_circuits(boxes: list[tuple[int]], iterations: int) -> tuple[int, int]:
    ...
    for i, (p, q, _) in enumerate(sorted(distances, key=lambda x: x[2])):
        ...
        if i == iterations -1:
            ...
            prod = lengths[0] * lengths[1] * lengths[2]

        if len(circuits) == 1 and len(circuits[0]) == len(boxes):
            last_cable = p[0] * q[0]
            break

    return prod, last_cable
...
with open("2025/day-08/example.txt", encoding="utf-8") as f:
    ...
    prod, last_cable = connect_circuits(boxes, 10)

    assert 40 == prod
    assert 25272 == last_cable

with open("2025/day-08/input.txt", encoding="utf-8") as f:
    input_lines = [line.strip() for line in f.readlines()]
    boxes = [tuple(int(pos) for pos in line.split(",")) for line in input_lines]

    prod, last_cable = connect_circuits(boxes, 1000)

    print(f"Part 1: The prod of the sizes of the three largest circuits is {prod}")
    print(f"Part 2: The prod of x coordinates of the last cable is {last_cable}")
